...
public class MainController {
	...
	public String index(@RequestParam(name = "idCategoria", required = false) Long idCategoria, Model model) {
		...
		/*
		 * Si la categoría no es nula, obtenemos 8 productos aleatoriamente, si es nula
		 * obtenemos todos los de la categoría con ese id, el llamado idcategoria
		 */
		if (idCategoria == null) {
			producto = productoService.obtenerProductosAleatorios(NUM_PRODUCTOS_ALEATORIOS);
		} else {
			producto = productoService.findAllByCategoria(idCategoria);
		}
